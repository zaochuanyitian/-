...
assert.match(service, /ROUND_OPTIONS = \[1, 4, 8, 16, 24\]/);
assert.match(game, /var roundChoice = 1/);
assert.match(indexHtml, /game\.js\?v=/);
assert.match(indexHtml, /game\.css\?v=/);
assert.match(adapters, /name: "玩家"/);
assert.match(adapters, /name: "对家甲"/);
assert.match(game, /保存昵称/);
assert.doesNotMatch(game, /小猫/);
assert.doesNotMatch(adapters, /小猫/);
